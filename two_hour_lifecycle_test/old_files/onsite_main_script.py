...
def calibration_cycle(GNSS_ser, TRX_ser, bat_level, temperature, start_t,  file_number, t,tres):
	#N = 18 # 18*5 = 90 mins
	...
	while datetime.now() <= (start_t + delta):
		...
		if command == "None":
			...
			if mode == 'normal':
				normal_ops(TRX_ser, min_az, max_az, min_el, max_el, file_number,tres)
			else:
				time.sleep(5*60) # 5 mins

def normal_ops(TRX_ser, min_az, max_az, min_el, max_el, file_number,tres):
#	time res idea: read in all available files? divide number of files by 
	...
	heights = reflector_height(path + dinofile, min_az, max_az, min_el, max_el,tres)
	...
	message = end_time + ";B=" + bat_level + ";T=" + temperature + ";H=" + heights[0] + heights[1]
...
def main():
    ...
    print(TRX_ser)
    # Check mailbox 
    command = "None" #check_mail() # 0 if no mail
    if command != "None":
        freq, min_el, max_el, min_az, max_az, mode, temporal_res = command_interpreter(command)

    else:
        set_vars = read_file('/home/velociraptor/two_hour_lifecycle_test/setvars.txt')
        freq = set_vars[0]
    ...
    print(mode)
    if mode == 'calibration':
        calibration_cycle(GNSS_ser, TRX_ser, bat_level, temperature, start_t, file_number, t,temporal_res)

    else: #normal ops (need to implement time resolution)
        normal_ops(TRX_ser, min_az, max_az, min_el, max_el, file_number,temporal_res)
