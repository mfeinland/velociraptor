...
def send_string(current_string, TRX_ser):
    ...
    while establish_connection_out != "OK":
        establish_connection_out = ser.readline()
        establish_connection_out = establish_connection_out.decode('utf-8').rstrip()
        print(establish_connection_out) # this should say "OK"
    ...
    if establish_connection_out == "OK": # if the serial connection is successful
        ...
        sbdwt_out = ser.readline(); print(sbdwt_out.decode('utf-8').rstrip()) # should say OK
        while sbdwt_out.decode('utf-8').rstrip() != "OK":
            sbdwt_out = ser.readline()
            stuff = sbdwt_out.decode('utf-8').rstrip()
            print(stuff)
...
def check_mail(TRX_ser):
    ...
    if check == "OK": # the serial connection is successful
      ...
      while str(err).strip() == str(32):
          time.sleep(10)
          cmd = "AT+SBDIX\r"
          TRX_ser.write(cmd.encode("ascii"))
          read_cmd = TRX_ser.readline()
          agg = TRX_ser.readline()
          print(read_cmd)
